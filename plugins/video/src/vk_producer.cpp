...
VkProducer::create(uint32_t width, uint32_t height, std::string* err) {
    ...
    app.pApplicationName = "waywallen-video-renderer";
     ...
     * the release timeline drm_syncobj. */
    ...
    // --- Command pool + buffer + per-submit fence ----------------------
...
int VkProducer::upload_into(VkImage target, uint32_t target_w, uint32_t target_h,
    ...
    /* Wait for the previous submit to finish before reusing the cmd
     * buffer + staging mapping. First call has no prior submit, so skip. */
    if (fence_pending_) {
    ...
    /* UNDEFINED → TRANSFER_DST_OPTIMAL. The bridge slot is fresh per
     * acquire so UNDEFINED is the right starting layout regardless of
     * whether the previous frame used it. */
